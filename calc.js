...
const i18n = {
    ...
    en: {
        ...
        fixed: "Auto Fill Free", reset: "Reset",
...
window.setFixedValues = function() {
    const msg = window.currentLang === 'ko' ? "매일 무료 획득량을 일괄 적용하시겠습니까?" : "Apply daily free settings?";
    if(confirm(msg)) {
...
window.resetDayData = function() {
    const msg = window.currentLang === 'ko' ? "초기화 하시겠습니까?" : "Reset data for this day?";
    if(confirm(msg)) {
...
window.openSpdModal = (cid, label) => { window.activeSpdId = `${window.currentDay}-${cid}`; const titleEl = document.getElementById('spd-title'); if(titleEl) titleEl.innerText = label; const savedSpd = window.getSpdData(window.activeSpdId); ['m5','m15','h1','h3','h8'].forEach(id => { const el = document.getElementById(id); if(el) el.value = savedSpd[id] || 0; }); window.calcSpdTotal(); document.getElementById('spdModal')?.classList.add('active'); };
...
function calculateDayScore(day) {
    ...
    } else if(day === 'sat') { score += parseFloat(getVal('sat-truck'))*BASE.truck*m.exp.all || 0; score += parseFloat(getVal('sat-sec'))*BASE.secret*m.exp.all || 0; score += parseFloat(getVal('sat-spd-all'))*60*BASE.spd_min*m.spd.sub || 0; let kScore = document.getElementById('sat-target')?.value === 'special' ? BASE.kil_spec[val('sat-elvl')] : BASE.kil_gen[val('sat-elvl')]; score += parseFloat(getVal('sat-kill'))*kScore*m.kil.sub || 0; score += parseFloat(getVal('sat-dth'))*BASE.trp[getVal('sat-alvl')||8]*m.exp.all || 0; }
    return score;
}
...
window.updateAll = function() {
    ...
    else if(d==='sat') { setPt('truck', val('sat-truck')*BASE.truck*m.exp.all); setPt('sec', val('sat-sec')*BASE.secret*m.exp.all); setPt('spd-all', val('sat-spd-all')*60*BASE.spd_min*m.spd.sub); let kScore = document.getElementById('sat-target')?.value === 'special' ? BASE.kil_spec[val('sat-elvl')] : BASE.kil_gen[val('sat-elvl')]; setPt('kill', val('sat-kill')*kScore*m.kil.sub); setPt('dth', val('sat-dth')*BASE.trp[val('sat-alvl')]*m.exp.all); }
    ...
    // 진행 바 달성 시 CSS 클래스로 애니메이션 연동
    const barEl = document.getElementById('bar');
    if(barEl) {
        barEl.style.width = pct + '%';
        if (pct >= 100) barEl.classList.add('completed');
...
window.showWeeklyReport = function() {
    const days = ['mon','tue','wed','thu','fri','sat']; const currentT = i18n[window.currentLang]; let html = `<div class="weekly-container">`; let totalWeeklyScore = 0;
    days.forEach((d, idx) => {
        const dayScore = calculateDayScore(d); totalWeeklyScore += dayScore;
        html += `<div class="weekly-row"><span class="weekly-day" style="color: var(--${d});">${currentT.days[idx]}</span><span class="weekly-score">${Math.floor(dayScore).toLocaleString()} <span>pt</span></span></div>`;
    });
...
window.syncToCloud = async function() { const data = localStorage.getItem('lastwar_data'); const spdData = localStorage.getItem('lastwar_spd_data'); try { await db.collection('user_sync').doc(myId).set({ basic: JSON.parse(data), speed: JSON.parse(spdData), updatedAt: firebase.firestore.FieldValue.serverTimestamp() }); alert("☁️ 데이터가 클라우드에 안전하게 저장되었습니다!"); } catch(e) { alert("저장 실패: " + e.message); } };
...
function renderInputs() {
    ...
    if(window.currentDay === 'mon') { const sVal = getVal('mon-squads'); const gVal = getVal('mon-gather'); html += `<div class="input-group-compact"><div class="input-header"><span class="input-label-small">${t.inputs.squads}</span></div><select id="mon-squads" class="compact-input" style="background:transparent; border-bottom:2px solid var(--input-bg);" onchange="updateAll()">${[1,2,3,4,5].map(n => `<option value="${n}" ${sVal==n?'selected':''}>${n}${t.inputs.squads_unit}</option>`).join('')}</select></div><div class="input-group-compact"><div class="input-header"><span class="input-label-small">${t.inputs.gather}</span><span class="item-score-tag" id="pts-gather">0</span></div><input type="number" id="mon-gather" class="compact-input" min="0" value="${gVal}" oninput="updateAll()"></div>`; }
    ...
    (config[window.currentDay] || []).forEach(i => { const cid = `${window.currentDay}-${i.id}`; html += `<div class="input-group-compact"><div class="input-header"><span class="input-label-small">${i.l}</span><span class="item-score-tag" id="pts-${i.id}">0</span></div><input type="number" id="${cid}" class="compact-input" min="0" value="${getVal(cid)}" oninput="updateAll()">${i.isSpd ? `<button class="spd-btn-mini" onclick="openSpdModal('${i.id}','${i.l}')">${t.modal.btn_open}</button>` : ''}</div>`; });

    if(window.currentDay === 'fri') { const lvl = getVal('fri-lvl') || "8"; html += `<div class="input-group-compact"><div class="input-header"><span class="input-label-small">${t.inputs.trn_lvl}</span></div><select id="fri-lvl" class="compact-input" style="background:transparent;" onchange="updateAll()">${Array.from({length:10},(_,i)=>`<option value="${i+1}" ${i+1==lvl?'selected':''}>Lv ${i+1}</option>`).join('')}</select></div><div class="input-group-compact"><div class="input-header"><span class="input-label-small">${t.inputs.trn_cnt}</span><span class="item-score-tag" id="pts-count">0</span></div><input type="number" id="fri-count" class="compact-input" min="0" value="${getVal('fri-count')}" oninput="updateAll()"></div>`; }
    if(window.currentDay === 'sat') { const target = getVal('sat-target') || "special"; html += `<div class="input-group-compact"><div class="input-header"><span class="input-label-small">${t.inputs.kill_target}</span></div><select id="sat-target" class="compact-input" style="background:transparent;" onchange="updateAll()"><option value="special" ${target=='special'?'selected':''}>${t.inputs.target_spec}</option><option value="general" ${target=='general'?'selected':''}>${t.inputs.target_gen}</option></select></div><div class="input-group-compact"><div class="input-header"><span class="input-label-small">${t.inputs.kill_lvl}</span></div><select id="sat-elvl" class="compact-input" style="background:transparent;" onchange="updateAll()">${Array.from({length:10},(_,i)=>`<option value="${i+1}" ${i+1==(getVal('sat-elvl')||8)?'selected':''}>Lv ${i+1}</option>`).join('')}</select></div><div class="input-group-compact"><div class="input-header"><span class="input-label-small">${t.inputs.kill_cnt}</span><span class="item-score-tag" id="pts-kill">0</span></div><input type="number" id="sat-kill" class="compact-input" min="0" value="${getVal('sat-kill')}" oninput="updateAll()"></div><div class="input-group-compact"><div class="input-header"><span class="input-label-small">${t.inputs.dth_lvl}</span></div><select id="sat-alvl" class="compact-input" style="background:transparent;" onchange="updateAll()">${Array.from({length:10},(_,i)=>`<option value="${i+1}" ${i+1==(getVal('sat-alvl')||8)?'selected':''}>Lv ${i+1}</option>`).join('')}</select></div><div class="input-group-compact"><div class="input-header"><span class="input-label-small">${t.inputs.dth_cnt}</span><span class="item-score-tag" id="pts-dth">0</span></div><input type="number" id="sat-dth" class="compact-input" min="0" value="${getVal('sat-dth')}" oninput="updateAll()"></div>`; }
    html += `</div>`; container.innerHTML = html;
}

window.saveAllData = function() { const inputs = document.querySelectorAll('.compact-input'); const data = JSON.parse(localStorage.getItem('lastwar_data') || '{}'); inputs.forEach(input => { if(input.id) data[input.id] = input.value; }); localStorage.setItem('lastwar_data', JSON.stringify(data)); };
...
function initCalc() {
    ...
    if(techGrid) { const techs = [ {id:'t-expert', l:t.expert, def: 20, max: 20}, {id:'t-radar', l:t.radar, def: 6, max: 10}, {id:'t-spd', l:t.spd, def: 6, max: 10}, {id:'t-rec', l:t.rec, def: 6, max: 10}, {id:'t-con', l:t.con, def: 1, max: 10}, {id:'t-tec', l:t.tec, def: 1, max: 10}, {id:'t-trn', l:t.trn, def: 6, max: 10}, {id:'t-kil', l:t.kil, def: 6, max: 10} ]; const savedData = JSON.parse(localStorage.getItem('lastwar_data') || '{}'); techGrid.innerHTML = techs.map(item => { let currentVal = savedData[item.id] !== undefined ? parseInt(savedData[item.id]) : item.def; if(currentVal > item.max) currentVal = item.max; return `<div class="tech-item"><label style="font-size:0.85rem; margin-bottom:5px; font-weight:700;">${item.l}</label><select id="${item.id}" class="compact-input" style="background:transparent; border:none;" onchange="updateAll()">${Array.from({length:item.max + 1},(_,i)=>`<option value="${i}" ${i===currentVal?'selected':''}>Lv ${i}</option>`).join('')}</select></div>`; }).join(''); }
    ...
    renderDroneInputs(); renderInputs(); updateAll();
}
